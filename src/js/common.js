...
aBook.common = (function () {
    ...
    function createNavTab() {
        ...
        $(document).on("click",".sideClick",function (e) {
            ...
            $(xmlDoc).find("first").each(function () {
                var field = $(this);
                if (field.attr("id") == dataId) {
                    var secondText = field.children("second").attr("name");
                    if (!secondText || secondText.trim() == "") {
                        addNavTabItem(tag);
                        createContentBox(dataId, "../src/webContent/" + dataId + "/" + dataId + ".html");
                    } else {
